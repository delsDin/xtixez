...
export const RadarSkillsChart: React.FC = () => {
  const [activeCategory, setActiveCategory] = useState<'all' | 'dev' | 'ds' | 'autres'>('all');
  ...
  const activeSkillsList = useMemo<SkillItem[]>(() => {
    switch (activeCategory) {
      case 'dev':
        return skillsData.development;
      case 'ds':
        return skillsData.dataScience;
      case 'autres':
        return skillsData.autres;
      case 'all':
      default:
        // A handpicked diverse set of top representational skills for a full overview (exactly 6 items for hex layout)
        return [
    ...
    <div className="bg-slate-50/50 dark:bg-slate-900/40 border border-slate-200/50 dark:border-slate-800/80 rounded-3xl p-5 md:p-8 shadow-sm">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
        ...
        <div className="flex bg-slate-200/60 dark:bg-slate-950/45 p-1 rounded-xl border border-slate-200/30 dark:border-slate-800/60 flex-wrap">
          ...
          <button
            onClick={() => { setActiveCategory('dev'); setHoveredSkill(null); }}
            className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold cursor-pointer transition-all ${
              activeCategory === 'dev' 
          ...
          <button
            onClick={() => { setActiveCategory('ds'); setHoveredSkill(null); }}
            className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold cursor-pointer transition-all ${
              activeCategory === 'ds' 
                ? 'bg-orange-550 bg-orange-500 text-white shadow-sm' 
                : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200'
            }`}
          >
            Data Science
          ...
          <button
            onClick={() => { setActiveCategory('autres'); setHoveredSkill(null); }}
            className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold cursor-pointer transition-all ${
