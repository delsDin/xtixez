import { fetchPortfolioConfig, fetchMaintenanceConfig, saveMaintenanceConfig, fetchIncidentLogs, clearIncidentLogs, settleIncidentLog } from '../lib/config-api';
...
export const AdminDashboard: React.FC = () => {
  ...
  const handleSimulateIncident = async (severity: 'info' | 'warning' | 'critical') => {
    try {
      showStatus(`Simulation d'un incident ${severity}...`, "info");
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
      try {
        ...
        console.error("Failed to fetch admin_notifications:", err);
      }
    };
    ...
    const channel = supabase.channel('admin_notifications_changes')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'admin_notifications' }, payload => {
        ...
        playNotificationSound(newNotif.type as any);
      })
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'admin_notifications' }, payload => {
        if (!isSubscribed) return;
        setAdminNotifications(prev => prev.map(n => n.id === payload.new.id ? { ...n, read: payload.new.is_read } : n));
      })
      ...
      >
        <div className="bg-slate-950 border border-slate-800 rounded-3xl w-full max-w-6xl shadow-2xl relative overflow-hidden flex flex-col max-h-[92vh]">
          ...
          <div className="relative border-b border-slate-850 bg-slate-950/90 px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 z-20">
            ...
            {/* Desktop and Tablet horizontal tabs layout (hidden below xl) */}
            <div className="hidden xl:flex flex-wrap items-center gap-2">
              {adminTabs.map((tab) => {
                ...
                let activeStyles = 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 shadow-md shadow-emerald-500/5';
                if (isContactMessages) {
                  activeStyles = 'bg-rose-500/10 text-rose-400 border border-rose-500/20 shadow-md shadow-rose-500/5';
                }
                ...
                return (
                  <button
                    ...
                    className={`relative px-3 py-1.5 font-mono text-[10px] md:text-[11px] font-black tracking-wider uppercase rounded-xl transition-all cursor-pointer ${
                      isActive
                        ? activeStyles
                  ...
                  >
                    {tab.label}
                    {unreadCount > 0 && (
                      <span className="absolute -top-1.5 -right-1.5 bg-rose-500 text-white font-mono text-[8px] font-normal leading-none px-1.5 py-0.5 rounded-full flex items-center justify-center animate-pulse">
                        {unreadCount}
                      </span>
                    )}
                  </button>
            ...
            {/* Hamburger controller for responsive navigation (shown below xl) */}
            <div className="xl:hidden flex items-center relative z-30">
              <button
                type="button"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="p-5 rounded-2xl bg-slate-900/40 border border-slate-800 flex flex-col h-[280px]">
                ...
                <div className="flex-grow overflow-auto p-3.5 bg-slate-950 border border-slate-800 rounded-xl font-mono text-[10px] leading-relaxed text-slate-400 select-all scrollbar-thin">
                  ...
                  <div>
                    <span className="text-emerald-500">APP_URL</span> = <span className="text-slate-200">"{window.location.origin}"</span>
                  </div>
                  <div className="mt-1">
