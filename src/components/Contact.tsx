...
export const Contact = () => {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      // 1. Stocke le message directement dans Supabase
      const { error } = await supabase.from('contact_messages').insert({
        id: `cmsg_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`,
        name: data.name,
        email: data.email,
      ...
      if (error) {
        console.warn("La persistance du message sur Supabase a échoué :", error.message);
      } else {
      ...
      console.error("Erreur lors de la validation du formulaire de contact:", error);
              ...
              <div className="space-y-6">
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-base sm:text-lg font-bold text-slate-900 dark:text-white group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors">{generalInfo?.owner_email || "delsmarceldinla@gmail.com"}</p>
                  </div>
                </motion.a>
                
                <motion.a 
                  href={`tel:${generalInfo?.owner_phone || "+2290153024367"}`} 
                  whileHover={{ x: 6, scale: 1.01 }}
                  transition={{ type: 'spring', stiffness: 350, damping: 25 }}
                  className="flex items-center gap-4 group cursor-pointer"
                >
                  ...
                  <div>
                    ...
                    <p className="text-base sm:text-lg font-bold text-slate-900 dark:text-white group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors">{generalInfo?.owner_phone || "+229 01 53 02 43 67"}</p>
                  </div>
                </motion.a>
                
                <motion.a 
                  href={`https://maps.google.com/?q=${encodeURIComponent(generalInfo?.owner_location || "Abomey-Calavi, Bénin")}`} 
                  target="_blank" 
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-base sm:text-lg font-bold text-slate-900 dark:text-white group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors">{generalInfo?.owner_location || "Abomey-Calavi, Bénin"}</p>
                  </div>
                </motion.a>
              </div>
            </div>
