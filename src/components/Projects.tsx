      ...
      <div className="container mx-auto px-4 md:px-6 max-w-6xl">
        ...
        >
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-6">
            {['Tous', ...categoriesList].filter(cat => cat === 'Tous' || projectsList.some(p => p.category === cat)).map(cat => (
              <motion.button
                key={cat}
          ...
          <motion.div layout className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            <AnimatePresence>
              {displayedProjects.map((project) => (
                ...
                >
                  <div className="w-full bg-white dark:bg-slate-900 rounded-xl shadow-md hover:shadow-xl hover:shadow-accent/5 border border-slate-200 dark:border-slate-700 hover:border-accent/40 dark:hover:border-accent/50 overflow-hidden flex flex-col transition-all duration-300">
                      ...
                      <div className="p-6 flex-1 flex flex-col">
                        ...
                        <div className="flex flex-wrap gap-2 mb-4">
                          {project.techs.slice(0, 3).map((tech: string) => (
                            <span key={tech} className="px-2.5 py-1 text-[11px] font-bold rounded-lg bg-amber-50 dark:bg-amber-950/40 text-amber-600 dark:text-amber-400 border border-amber-500/20">
                              {tech}
                            </span>
                          ))}
                          {project.techs.length > 3 && (
                            <span className="px-2.5 py-1 text-[11px] font-bold rounded-lg bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
                              +{project.techs.length - 3}
                            </span>
                          )}
                        </div>
